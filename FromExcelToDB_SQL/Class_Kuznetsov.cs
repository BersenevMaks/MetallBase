    ...
    class Class_Kuznetsov
    {
        ...
        private void ReadWord()
        {
            ...
            try
            {
                ...
                int lastRow = 0;
                int ColName = 0, ColRaz = 0, ColMera = 0, ColMark = 0, ColPrice = 0;
                C_InfoTable tab;
                    ...
                    if (ColName != 0)
                    {
                        for (int jj = tab.StartRow + 1; jj <= cCelRow; jj++) //строки
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                {
                                    tolsh = "";
                                    tolsh = new Regex(@"(?<=\s)\d+(?:[,\.]\d+)?(?=\s*мм|\s*$)", RegexOptions.IgnoreCase).Match(temp).Value;

                                    temp = wTab.Cell(jj, ColRaz).Range.Text;
                                    temp = temp.Replace("\r\a", string.Empty).Trim();
                                    if (temp != "")
                                    {
                                        temp = temp.Replace(" ", "");
                                        ...
                                        diam = ""; metraj = "";
                                        if (new Regex(@"\d+(?:[,\.]\d+)?\s*[xх]\s*\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                        {
                                        ...
                                        else if (new Regex(@"\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                            metraj = new Regex(@"\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                        
                                        if (!String.IsNullOrEmpty(tolsh) || !String.IsNullOrEmpty(diam))
                                        {
                                            prim = temp;
                                            if (ColMark > 0)
                                            {
                                            ...
                                            row["Диаметр (высота), мм"] = tolsh;
                                            row["Толщина (ширина), мм"] = diam;
                ...
                for (int i = 1; i <= 10; i++)
                {
                    ...
                    if (temp != "")
                    {
                        ...
                        if (new Regex(@"(?<=тел[\/]факс\s*:?\s*)[\/\(]351[\/\)]\s*\d+-\d+-\d+", RegexOptions.IgnoreCase).IsMatch(temp))
                        {
                            if (String.IsNullOrEmpty(infoOrg.OrgTel)) infoOrg.OrgTel = new Regex(@"(?<=тел[\/]факс\s*:?\s*)[\/\(]351[\/\)]\s*\d+-\d+-\d+", RegexOptions.IgnoreCase).Match(temp).Value;
                            else infoOrg.OrgTel += "; " + new Regex(@"(?<=тел[\/]факс\s*:?\s*)[\/\(]351[\/\)]\s*\d+-\d+-\d+", RegexOptions.IgnoreCase).Match(temp).Value;
                        }
                        if (new Regex(@"(?<=об.*тел\s*:?\s*)\+\d+", RegexOptions.IgnoreCase).IsMatch(temp))
                        {
                            if(String.IsNullOrEmpty(infoOrg.OrgTel)) infoOrg.OrgTel = new Regex(@"(?<=об.*тел\s*:?\s*)\+\d+", RegexOptions.IgnoreCase).Match(temp).Value;
                            else infoOrg.OrgTel += "; " + new Regex(@"(?<=об.*тел\s*:?\s*)\+\d+", RegexOptions.IgnoreCase).Match(temp).Value;
                        }
                        if (new Regex(@"Склад\w?\s*:?\s*г\.\s*\w+", RegexOptions.IgnoreCase).IsMatch(temp))
                        {
                            infoOrg.SkladAdr.Add(new Regex(@"(?<=склад\w?\s*:?\s*)г\.\s*\w+(?:,?\s*ул\.\s*\w+)?\s*[\d\w]+", RegexOptions.IgnoreCase).Match(temp).Value);
                        }
                        if (new Regex(@"(?<=сполнитель\s*:\s*)(?:[\w]+\s?)+", RegexOptions.IgnoreCase).IsMatch(temp))
                        {
                            string telefon = " ";
                            string[] manager = new string[] { new Regex(@"(?<=сполнитель\s*:\s*)(?:[\w]+\s?)+", RegexOptions.IgnoreCase).Match(temp).Value, telefon };
