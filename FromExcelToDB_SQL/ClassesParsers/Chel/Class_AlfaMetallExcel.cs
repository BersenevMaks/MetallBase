...
namespace MetalBase.ClassesParsers.Chel
{
    class Class_AlfaMetallExcel
    {
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    for (int j = 1; j <= cCelRow; j++) //строки
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"наим\w*ие", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"вес.*свободный", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                    continue;
                                }
                            ...
                            if (ColName > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    if (regexParam.RegName.IsMatch(temp))
                                    {
                                        ...
                                        type = regexParam.GetTypeLongFromShort(type, name);
                                        temp = temp.ToLower().Replace("нд", "0");
                                        ...
                                        {
                                            //if (name.ToLower().Contains("пруток"))
                                            //{
                                            //    diam = new Regex(@"(?<=s|)\d+(?:[,.]\d+)?(?=[хx]?)", RegexOptions.IgnoreCase).Match(temp).Value;
                                            //}
                                            //else
                                            {
                                                //diamDxDxD
                                                ...
                                                else if (new Regex(@"\d+(?:[,.]\d+)?\s*[xх\*×]\s*(?:\d+(?:[,.]\d+)?)\s*", RegexOptions.IgnoreCase).IsMatch(temp))
                                                {
                                                    ...
                                                    try
                                                    {
                                                        if (double.TryParse(tolsh, out dT) && double.TryParse(diam, out dD) && name.ToLower() != "пруток")
                                            ...
                                            if (!string.IsNullOrEmpty(standart))
                                                temp = temp.Replace(standart, "").Trim();
                                            if (string.IsNullOrEmpty(metraj))
                                            {
                                                metraj = new Regex(@"(?<=,\s+)\d+(?:[,\.]\d+)?(?=\s*м)", RegexOptions.IgnoreCase).Match(temp).Value;
                                                if (!string.IsNullOrEmpty(metraj))
                                                    temp = temp.Replace(metraj, "").Trim();
                                            }
        ...
        {
            if (new Regex(@"^\s*г.\w+,?\s*[\w\s\.]+,\s*[\d\-]+", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgAdress = new Regex(@"\s*г.\w+,?\s*[\w\s\.]+,\s*[\d\-]+", RegexOptions.IgnoreCase).Match(temp).Value;
            }
            if (new Regex(@"тел\.?[\(\d\)\-\s]+", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgTel = new Regex(@"тел\.?[\(\d\)\-\s]+", RegexOptions.IgnoreCase).Match(temp).Value;
            }
