...
{
    class Class_UralTeploEnergoServiceExcel
    {
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            if (ColName > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    //поиск типа и его удаление, если найден
                                    type = regexParam.GetTypeLongFromShortEndNull(type, name);
                                    ...
                                    //поиск диаметра, толщины и метража
                                    dtm.CalcDTM(temp);
                                    diam = dtm.D();
                                    tolsh = dtm.T();
                                    metraj = dtm.M();
                                    if (regexParam.DiamDxDxD_Only.IsMatch(temp))
                                    { temp = temp.Replace(regexParam.DiamDxDxD_Only.Match(temp).Value, string.Empty); }
                                    else if (regexParam.DiamDxD_Only.IsMatch(temp))
                                    {
                                        temp = temp.Replace(regexParam.DiamDxD_Only.Match(temp).Value, string.Empty);
                                    }
                                ...
                                if (ColPrice > 0)
                                {
                                    ...
                                    {
                                        temp = temp.Replace(".", ",");
                                        price = new Regex(@"\d+(?:[,.\s]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                    }
        ...
        private static void FillInfoOrg(InfoOrganization infoOrg, string temp, C_RegexParamProduct regexParam, Excel.Worksheet worksheet, int j, int i)
        {
            ...
            else if (string.IsNullOrEmpty(infoOrg.OrgAdress))
            {
                infoOrg.OrgAdress = new Regex(@"(?:\d{6}\s*)?(г\.\s*\w+,?)?(?:\s*(\w+\s)?(?:ул|пр(?:-к?т)?|д|оф)\.?\s*\w+(?:\s*\w+)?,?)+(?:\s*кв\.\s*\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
            }

            if (regexParam.OrgMobileTelefon_tel_XXXXXXXXXXX.IsMatch(temp))
            {
                infoOrg.OrgTel = regexParam.OrgMobileTelefon_tel_XXXXXXXXXXX.Match(temp).Value;
            }
