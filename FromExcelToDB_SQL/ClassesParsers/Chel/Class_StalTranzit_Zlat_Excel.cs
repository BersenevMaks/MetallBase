...
{
    class Class_StalTranzit_Zlat_Excel
    {
        ...
        private void ReadExcel()
        {
            ...
            try
            {
                orgname = "Сталь Транзит Златоуст";
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = tab.StartCol; i <= endCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                //    continue;
                                //}
                                if (new Regex(@"\bкон.*?остаток\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                    continue;
                                }
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        ...
                                        temp = new Regex(@"(?<=\s*?)[\dxх]+(?=\s*?мм)", RegexOptions.IgnoreCase).Match(temp).Value;
