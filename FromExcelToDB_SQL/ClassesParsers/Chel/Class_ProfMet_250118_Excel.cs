...
{
    class Class_ProfMet_250118_Excel
    {
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                {
                                    ColName = i;
                                    if (cellRange.MergeArea.Rows.Count > 1) jjj += cellRange.MergeArea.Rows.Count - 1;
                                    continue;
                                }
                                if (new Regex(@"остаток", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    cellRange = (Excel.Range)excelworksheet.Cells[jjj - 1, i];
                                    if (cellRange.MergeArea.Rows.Count > 1) cellRange = (Excel.Range)excelworksheet.Cells[cellRange.MergeArea.Row, i];
                                    if (cellRange.Value != null)
                                        temp = cellRange.Value.ToString().Trim();
                                    else temp = "";
                                    if (temp != "")
                                    {
                                        if (temp.ToLower().Contains("профмет")) ColPrice = i;
                                    }
                                    continue;
                                }
