...
{
    class Class_ZolotoyVek240719_Excel
    {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (regexParam.RegTU.IsMatch(temp))
                                    {
                                        ...
                                        temp = temp.Replace(tab.Standart, "");
                                    }
                                    ...
                                    if (!string.IsNullOrEmpty(tab.Name))
                                    {
                                        ...
                                        if (string.IsNullOrEmpty(metraj))
                                            if (new Regex(@"\d+\s*?м", RegexOptions.IgnoreCase).IsMatch(temp))
                                                metraj = new Regex(@"\d+\s*?м", RegexOptions.IgnoreCase).Match(temp).Value;
                                        if (!string.IsNullOrEmpty(metraj)) temp = temp.Replace(metraj, "");


                                        if (temp.Length > 1)
                                        {
                                            price = new Regex(@"\d+(?=\s*за)", RegexOptions.IgnoreCase).Match(temp).Value;
                                            if(string.IsNullOrEmpty(price))
                                            {
                                                double p = 0;
                                                try
                                                {
                                                    foreach (Match m in new Regex(@"\d+(?:[,\.]\d+)?").Matches(temp))
                                                    {
                                                        p = Convert.ToDouble(m.Value);
                                                        if (p >= 100)
                                                            price = p.ToString();
                                                    }
                                                }
                                            ...
                                            if (!string.IsNullOrEmpty(mera)) temp = temp.Replace(mera, "");
