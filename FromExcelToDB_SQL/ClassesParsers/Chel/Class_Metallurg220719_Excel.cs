...
{
    class Class_Metallurg220719_Excel
    {
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    for (int j = 1; j <= cCelRow; j++) //строки
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"диаметр\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(tab.Name))
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(diam))
                                            temp = temp.Replace(dtm.DTM().Match(temp).Value, "");
                                        ...
                                        if (ColGost > 0)
                                        {
                                            ...
                                            if (temp != "")
                                            {
                                                ...
                                                if (string.IsNullOrEmpty(metraj))
                                                    if (new Regex(@"\d+\s*?м", RegexOptions.IgnoreCase).IsMatch(temp))
                                                        metraj = new Regex(@"\d+(?:[,\.]\d+)?\s*?м", RegexOptions.IgnoreCase).Match(temp).Value;   
                                            }
                                        }
                                        ...
                                        if (ColMark > 0)
                                        {
                                            ...
                                            if (temp != "")
                                            {
                                                ...
                                                {
                                                    mark = new Regex(@"\w*\d+\w*",RegexOptions.IgnoreCase).Match(temp).Value;
                                                }
                                            }
                                        }
