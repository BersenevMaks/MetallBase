        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    int ColDiam = 0, ColMera = 0, ColGost = 0, ColDlina = 0, ColTolsh = 0, ColMark = 0, ColName = 0, ColPrice = 0;
                    ...
                    {
                        ColGost = 0; ColDiam = 0; ColMera = 0; ColTolsh = 0; ColPrice = 0; ColName = 0;
                        name = ""; type = "";
                        Excel.Range cellRange;
                        tab = tabs[k];
                        int endRow = cCelRow;
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"масса", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                    continue;
                                }
                        ...
                        {
                            diam = ""; tolsh = ""; metraj = ""; mera = ""; price = ""; name = "Труба"; prim = ""; standart = "";
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    diam = new Regex(@"\d+(?:[,\.]\s?\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                    diam = diam.Replace('.', ',');
                                    ...
                                    if (ColTolsh > 0)
                                    {
                                        ...
                                        {
                                            prim += " стенка=" + temp;
                                            tolsh = new Regex(@"\d+(?:[,\.]\s?\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                            tolsh = tolsh.Replace('.', ',');
                                        }
                                    }
                                    ...
                                    if (ColDlina > 0)
                                    {
                                        ...
                                        {
                                            prim += " L=" + temp;
                                            metraj = new Regex(@"\d+(?:[,\.]\s?\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
