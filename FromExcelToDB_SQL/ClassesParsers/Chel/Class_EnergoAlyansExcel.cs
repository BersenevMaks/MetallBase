...
namespace MetalBase.ClassesParsers.Chel
{
    class Class_EnergoAlyansExcel
    {
        ...
        {
            if (new Regex(@"адрес\s*:", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgAdress = new Regex(@"(?<=дрес\s*:\s?).*", RegexOptions.IgnoreCase).Match(temp).Value.Trim();
            }
