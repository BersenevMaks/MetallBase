...
{
    class Class_MetalBrendUralExcel
    {
        ...
        private static void FillInfoOrg(InfoOrganization infoOrg, string temp, C_RegexParamProduct regexParam, Excel.Worksheet worksheet, int j, int i)
        {
            ...
            if (new Regex(@"(?<=Тел/факс:\s*).*-\d{3}(?=\s)", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgTel = new Regex(@"(?<=Тел/факс:\s*).*-\d{3}(?=\s)", RegexOptions.IgnoreCase).Match(temp).Value;
            }
            if (new Regex(@"\+\d{11}", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgTel += "," + new Regex(@"\+\d{11}", RegexOptions.IgnoreCase).Match(temp).Value;
            }
