                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            for (int rows = jjj; rows <= jjj + cellRange.MergeArea.Rows.Count; rows++)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (new Regex(@"(в\s)?наличи[ие]", RegexOptions.IgnoreCase).IsMatch(temp))
                                    {
                                        ColMera = i;
                                        continue;
                                    }
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            if (ColName > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    {
                                        type = new Regex(@"оцин|эл[/\\]св", RegexOptions.IgnoreCase).Match(temp).Value;
                                        if (!string.IsNullOrEmpty(type))
                                            type = regexParam.GetTypeLongFromShort(type, name);
                                    }
                                    if (!string.IsNullOrEmpty(type)) temp = temp.Replace(type, "");
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        temp = temp.Replace(name, "");
                                        if (name.ToLower().Contains("болты"))
                                        {
                                            name = "Болт";
                                            if (type.ToLower().Contains("полная")) type = "";
                                        }
                                        if (name.ToLower().Contains("шпильки")) name = "Шпилька";
                                        if (name.ToLower().Contains("гайки")) name = "Гайка";
                                        if (name.ToLower().Contains("шайбы")) name = "Шайба";
                                        if (name.ToLower().Contains("электроды")) name = "Электрод";
                                    }
                                    temp = temp.Trim();
                                    ...
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        ...
                                        if (new Regex(@"\d+(?:[,.]\d+)?\s*[xх\*×]\s*\d+(?:[,.]\d+)?\s*[xх\*×]\s*\d+(?:[,.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                        {
                                            ...
                                            if (new Regex(@"проф", RegexOptions.IgnoreCase).IsMatch(type))
                                        ...
                                        else if (new Regex(@"(?<=d\s+)\d+(?:[,.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                        {
                                            diam = new Regex(@"(?<=d\s+)\d+(?:[,.]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                        }
                                        else diam = new Regex(@"\d+(?:[,.]\d+)?(?=\s)", RegexOptions.IgnoreCase).Match(temp).Value;
