...
{
    class Class_Truba_Na_Sklade_Excel
    {
        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    int ColName = 0, ColDiam = 0, ColDlina = 0, ColMera = 0, ColGost = 0, ColTolsh = 0, ColMark = 0, ColPrice = 0;
                    ...
                    for (int j = 1; j <= cCelRow; j++) //строки
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"\bнаименование|\bдиаметр\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = tab.StartCol; i <= endCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"\bнаименование", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColName = i;
                                    continue;
                                }
                                if (new Regex(@"\bдиаметр", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColDiam = i;
                                    continue;
                                }
                                ...
                                if (new Regex(@"\bмарка", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMark = i;
                                    continue;
                                }
                                ...
                                if (new Regex(@"\bналичие\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                    continue;
                                }
                        ...
                        for (int j = tab.StartRow; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    prim += " " + temp;
                                    if (new Regex(@"швелл?ер|балка", RegexOptions.IgnoreCase).IsMatch(name))
                                    {
                                        type = regexParam.RegTypeShveller2.Match(temp).Value;
                                        if (string.IsNullOrEmpty(type)) type = regexParam.RegType.Match(temp).Value;
                                    }
                                    if (ColName > 0)
                                    {
                                        ...
                                        try
                                        {
                                            dtm.CalcDTM(temp, tab.Type, name: name);
                                            if (!string.IsNullOrEmpty(dtm.D()))
                                                dDiam.Add(Convert.ToDouble(dtm.D().Replace('.', ',').Trim()));
                                            tolsh = dtm.T();
                                            metraj = dtm.M();
                                        }
                                    ...
                                    {
                                        if(new Regex(@"ст.*,.*\d[хx]", RegexOptions.IgnoreCase).IsMatch(temp))
                                        {
                                            mark = new Regex(@"ст.*,.*\d[хx]", RegexOptions.IgnoreCase).Match(temp).Value;
                                            temp = temp.Replace(mark, "").Trim();
                                        }
                                        else
                                        foreach (Match m in regexParam.RegMark.Matches(temp))
                                            if (string.IsNullOrEmpty(mark)) { mark = m.Value; temp = temp.Replace(mark, "").Trim(); }
                                            else { mark += ", " + m.Value; temp = temp.Replace(m.Value, "").Trim(); }
                                        if(string.IsNullOrEmpty(mark))
                                        {
                                            mark = new Regex(@"ст\.[\d+[хх]\s,]+", RegexOptions.IgnoreCase).Match(temp).Value;
                                                }
                                        prim += ", " + mark;
                                        if (new Regex(@"\d+(?:[,\.]\d+)?\s*-\s*\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                            tmp = new Regex(@"\d+(?:[,\.]\d+)?\s*-\s*\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                        string[] diams = new Regex(@"(?<=\d+(?:[,\.]\d+)?)\s*-\s*(?=\d+(?:[,\.]\d+)?)", RegexOptions.IgnoreCase).Split(tmp);
                                        if (diams.Length > 1)
                                            dDiam = GetIncrementingMassiv(diams);
                                        else
                                        {
                                            diam = new Regex(@"\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                            if(!string.IsNullOrEmpty(diam))
                                                dDiam.Add(Convert.ToDouble(diam.Replace('.', ',').Trim()));
                                        }
                                    }
                                    ...
                                    if ((!String.IsNullOrEmpty(diam) || dDiam.Count>0) && (!String.IsNullOrEmpty(name) || !string.IsNullOrEmpty(tab.Name)))
                                    {
                                        if (dDiam.Count > 0)
                                            foreach (double d in dDiam)
                                            {
        ...
        private static void FillInfoOrg(InfoOrganization infoOrg, string temp, C_RegexParamProduct regexParam, Excel.Worksheet worksheet, int j, int i)
        {
            ...
            if (new Regex(@"\d{3}(?:\s?\-\s?\d{2}){2}", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                foreach (Match m in new Regex(@"\d{3}(?:\s?\-\s?\d{2}){2}", RegexOptions.IgnoreCase).Matches(temp))
                {
                    if (string.IsNullOrEmpty(infoOrg.OrgTel)) infoOrg.OrgTel = m.Value;
                    else infoOrg.OrgTel += ", " + m.Value;
                }
            ...
            if (new Regex(@"\+[\d\(\)\-]{11,}\s+\w+(?:\s+\w+)?", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                string[] manager = new string[]
                {
                    new Regex(@"(?<=\+[\d\(\)\-]{11,}\s+)\w+(?:\s+\w+)?", RegexOptions.IgnoreCase).Match(temp).Value, //имя
                    new Regex(@"\+[\d\(\)\-]{11,}(?=\s+\w+(?:\s+\w+)?)", RegexOptions.IgnoreCase).Match(temp).Value, //телефон
                    "" //email
                };
