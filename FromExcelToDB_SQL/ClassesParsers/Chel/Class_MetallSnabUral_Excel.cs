...
{
    class Class_MetallSnabUral_Excel
    {
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = tab.StartCol; i <= endCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"\bопт.*?цен\w\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColPrice = i;
                                    continue;
                                }
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        ...
                                        if(new Regex(@"\d+(?:,\.\d+)?(?:\*\d+(?:,\.\d+)?)+", RegexOptions.IgnoreCase).IsMatch(temp))
                                        {
                                            temp = temp.Replace('*', 'x');
                                        }
                                        ...
                                        if (!string.IsNullOrEmpty(diam) && !string.IsNullOrEmpty(tolsh) && !string.IsNullOrEmpty(metraj) && 
                                            !name.ToLower().Contains("лист") && !name.ToLower().Contains("полоса"))
                                            try
                                            {
                                        ...
                                        if (string.IsNullOrEmpty(metraj))
                                            if (new Regex(@"\b\d+(?:[,\.]\d+)?[mм]\b|бухта", RegexOptions.IgnoreCase).IsMatch(temp))
                                            {
                                                metraj = new Regex(@"\b\d+(?:[,\.]\d+)?[mм]\b|бухта", RegexOptions.IgnoreCase).Match(temp).Value;
                                            }
                                    }
                                    ...
                                    if (ColPrice > 0)
                                    {
                                        ...
                                        if (temp != "")
                                        {
                                            price = new Regex(@"[\s\d]+(?:,\.[\s\d]+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                        }
                                    }
