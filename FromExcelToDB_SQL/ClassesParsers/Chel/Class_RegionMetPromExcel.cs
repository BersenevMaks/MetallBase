...
{
    class Class_RegionMetPromExcel
    {
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    for (int j = 1; j <= cCelRow; j++) //строки
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"номенклатура\s*,\s*баз", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            if (ColName > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(name)) temp = temp.Replace(name, "");
