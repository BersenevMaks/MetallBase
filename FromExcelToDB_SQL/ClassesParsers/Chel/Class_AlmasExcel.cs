                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"\bту,\s*гост\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColGost = i;
                                    continue;
                                }
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    if (cellRange.MergeArea.Columns.Count > 1)
                                    {
                                        ...
                                        {
                                            type = new Regex(@"(?<=\w+)(?:ые|ое|ая)", RegexOptions.IgnoreCase).Replace(temp, "ая");
                                            tab.Type += m.Value + " ";
                                        }
                                        tab.Standart = regexParam.RegMark.Match(temp).Value;
                                        
                                    }
                                    else
                                    {
                                        temp = temp.Replace(".", ",");
                                        dtm.CalcDTM(temp, name: name);
                                        diam = dtm.D();
                                        tolsh = dtm.T();
                                        metraj = dtm.M();
                                        ...
                                        if (regexParam.RegType.IsMatch(temp))
                                        {
                                            ...
                                            type = regexParam.GetTypeLongFromShort(type, tab.Name);
                                        }
                                    }
                                    prim = tab.Name + "  ";
                                    if (!string.IsNullOrEmpty(tab.Type)) prim += tab.Type + "  ";
                                }
                            }
                            if (ColTolsh > 0)
                            {
