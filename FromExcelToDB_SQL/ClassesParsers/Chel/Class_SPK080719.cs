        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                        ...
                        for (int j = tab.StartRow; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    name = regexParam.RegName.Match(listname).Value;
                                    if (string.IsNullOrEmpty(name))
                                        if (new Regex(@"пвл", RegexOptions.IgnoreCase).IsMatch(listname))
                                            name = "Лист";
                                    prim = temp;
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        for (int ti = 1; ti < temps.Length; ti++)
                                        {
                                            if(!string.IsNullOrEmpty(regexParam.RegTypeShort(temps[ti],name)))
                                            {
                                                if(string.IsNullOrEmpty(type)) type = regexParam.RegTypeShort(temps[ti], name);
                                                else type += ", " + regexParam.RegTypeShort(temps[ti], name);
                                                continue;
                                            }
                                            else
                                            {
                                                temps[ti] = temps[ti].Replace('.', ',');
                                                if (new Regex(@"швелл?ер|балка", RegexOptions.IgnoreCase).IsMatch(name))
                                                {
                                                    if (new Regex(@"(?<=\d)(?:б|ш|kк|[уy]|п)", RegexOptions.IgnoreCase).IsMatch(temps[ti]))
                                                    {
                                                        type = new Regex(@"(?<=\d)(?:б|ш|kк|[уy]|п)", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                        diam = new Regex(@"\d+(?:[,\.]\d+)?(?=(?:б|ш|kк|[уy]|п))", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                    }
                                                }
                                                if (new Regex(@"^D?[1234567890,]+$|^\d+(?:,\.\d+)?\s?ДУ",RegexOptions.IgnoreCase).IsMatch(temps[ti]))
                                                {
                                                    if (string.IsNullOrEmpty(diam))
                                                        diam = new Regex(@"\d+(?:,\.\d+)?", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                    else if (string.IsNullOrEmpty(tolsh))
                                                        tolsh = new Regex(@"\d+(?:,\.\d+)?", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                    else if (string.IsNullOrEmpty(metraj))
                                                        metraj = new Regex(@"\d+(?:,\.\d+)?", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                ...
                                                if (string.IsNullOrEmpty(metraj))
                                                    if (new Regex(@"\b\d+(?:[,\.]\d+)?[mм]\b|бухта", RegexOptions.IgnoreCase).IsMatch(temps[ti]))
                                                    {
                                                        metraj = new Regex(@"\b\d+(?:[,\.]\d+)?[mм]\b|бухта", RegexOptions.IgnoreCase).Match(temps[ti]).Value;
                                                        continue;
                                                    }
                                            }
                                        }
                                        mark = regexParam.RegMark.Match(temps[temps.Length - 1]).Value;
                                        if (string.IsNullOrEmpty(mark))
                                        {
                                            if(temps.Length>1)
                                            mark = regexParam.RegMark.Match(temps[temps.Length - 2]).Value;
                                        }
                                    }
                    ...
                    if (tabs.Count > 0 && dtProduct.Rows.Count > 0 && excelworksheet.Index == 2)
                    {
