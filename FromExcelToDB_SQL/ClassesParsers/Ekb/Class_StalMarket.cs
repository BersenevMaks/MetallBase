        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    int ColDlina = 0, ColMera = 0, ColPrice = 0, ColGost = 0, ColSoder = 0, ColDiam = 0, ColTolsh = 0, ColMark = 0;
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"\bрезерв\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                }
                        ...
                        for (int j = tab.StartRow - 1; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (cellRange.MergeArea.Columns.Count > 1)
                                {
                                    ...
                                    {
                                        name = "";
                                        if (regexParam.RegName.IsMatch(temp)) name = StringFirstUp(regexParam.RegName.Match(temp).Value);
                                        else name = "Труба";
                                        if (new Regex(@"(?<=(?:наличие|продукции)+.*\d\d/)\w+(?:\s*\(?\w+\))?", RegexOptions.IgnoreCase).IsMatch(temp))
                                        { skladPrim = new Regex(@"(?<=(?:наличие|продукции)+.*\d\d/)\w+(?:\s*\(?\w+\))?", RegexOptions.IgnoreCase).Match(temp).Value; }
                                    }
                                }
                                else
                                {
                                    ...
                                    if (temp != "")
                                    {
                                        ...
                                        prim = "Склад: \"" + skladPrim + "\", диам:" + diam;
                                        if (ColTolsh > 0)
                                        {
                                        ...
                                        if (ColMark > 0)
                                        {
                                            ...
                                            if (temp != "")
                                            {
                                                if (new Regex(@"^ст\.?", RegexOptions.IgnoreCase).IsMatch(temp))
                                                    mark = temp;
                                                else mark = "Ст." + temp;
                                            }
                                        }
