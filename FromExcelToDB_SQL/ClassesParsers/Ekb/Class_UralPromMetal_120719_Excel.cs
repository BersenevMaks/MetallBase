...
namespace MetalBase.ClassesParsers.Ekb
{
    class Class_UralPromMetal_120719_Excel
    {
        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    for (int j = 1; j <= 10; j++)//cCelRow; j++) //строки
                    {
                        int jj = j;
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"номенклатура(?=\s*$)", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        int endCol = cCelCol;
                        if (k < tabs.Count - 1)
                            if (tabs[k + 1].StartCol > tab.StartCol)
                                endCol = tabs[k + 1].StartCol;
                            else endCol = cCelCol;
                        for (int i = tab.StartCol; i < endCol; i++) //столбцы
                        {
                            ...
                            if(cellRange.MergeArea.Rows.Count>1)
                                cellRange = (Excel.Range)excelworksheet.Cells[cellRange.Row, i];
                            if (cellRange.Value != null)
                                temp = cellRange.Value.ToString().Trim();
                            else temp = "";
                            if (temp != "")
                            {
                                ...
                                //    continue;
                                //}
                                if (new Regex(@"\bхарактеристика\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColDlina = i;
                                    continue;
                                }
                                ...
                                //if (new Regex(@"\bсталь\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                //{
                                //    ColMark = i;
                                //    continue;
                                //}
                        ...
                        for (int j = tab.StartRow; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                else if (cellRange.MergeArea.Rows.Count > 1)
                                {
                                    cellRange = (Excel.Range)excelworksheet.Cells[cellRange.MergeArea.Row, ColDiam];
                                    if (cellRange.Value != null)
                                        temp = cellRange.Value.ToString().Trim();
                                }
                                else temp = "";
                                if (temp != "")
                                {
                                    ...
                                    if (new Regex(@"\bшест(?:.+?р.+?ник)?\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                    {
                                        name = "Шестигранник";
                                        temp = temp.Replace(new Regex(@"\bшест(?:.+?р.+?ник)?\b", RegexOptions.IgnoreCase).Match(temp).Value, "");
                                    }
                                    if (string.IsNullOrEmpty(name)) tab.Name = "Труба";

                                    if (regexParam.RegType.IsMatch(temp))
                                    {
                                        type = "";
                                        
                                        if (!new Regex(@"\bдоговорн\w*?\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                            foreach (Match m in regexParam.RegType.Matches(temp))
                                                if (string.IsNullOrEmpty(type)) type = regexParam.SetRightEnding(m.Value, tab.Name);
                                                else type += ", " + regexParam.SetRightEnding(m.Value, tab.Name);
                                        if (!string.IsNullOrEmpty(type))
                                        {
                                            type = regexParam.SetRightEnding(type, tab.Name);
                                            if (cellRange.MergeArea.Columns.Count > 3) tab.Type = type;
                                        }
                                        else
                                        {
                                            type = regexParam.RegTypeShort(temp, tab.Name);
                                            if (!string.IsNullOrEmpty(type)) type = regexParam.GetTypeLongFromShort(type, name);
                                            if (cellRange.MergeArea.Columns.Count > 3) tab.Type = type;
                                        }
                                    }
                                    if (name.ToLower().Contains("двутавр") || name.ToLower().Contains("швел"))
                                    {
                                    ...
                                    if (!string.IsNullOrEmpty(name))
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(diam))
                                            temp = temp.Replace(dtm.DTM().Match(temp).Value, "").Trim();
                                        ...
                                        price = new Regex(@"\d+(?:[,\.]\d+)?(?=\s*$)", RegexOptions.IgnoreCase).Match(temp).Value;
                                        if (!string.IsNullOrEmpty(price)) temp = temp.Replace(price, "");



                                        //if (string.IsNullOrEmpty(metraj)) metraj = new Regex(@"(?<=дл\.\s*?)\d+(?:[,\.]\d+)?", RegexOptions.IgnoreCase).Match(temp).Value;
                                        ...
                                        if (ColDlina > 0)
                                        {
                                            ...
                                            if (temp != "")
                                            {
                                                ...
                                                {
                                                    metraj = new Regex(@"[\d-]+", RegexOptions.IgnoreCase).Match(temp).Value;
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    if (!String.IsNullOrEmpty(diam) && (!String.IsNullOrEmpty(name) || !string.IsNullOrEmpty(tab.Name)) && 
                                        (!string.IsNullOrEmpty(standart) || !string.IsNullOrEmpty(price) || !string.IsNullOrEmpty(mark)))
                                    {
        ...
        {
            if (new Regex(@"(?<=офис\s*:\s*).*?(?=\s\s\s)", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.OrgAdress = new Regex(@"(?<=офис\s*:\s*).*?(?=\s\s\s)", RegexOptions.IgnoreCase).Match(temp).Value;
            }
            ...
            if (new Regex(@"(?<=склад\s*:\s*).*?(?=\s\s\s)", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                infoOrg.SkladAdr.Add(new Regex(@"(?<=склад\s*:\s*).*?(?=\s\s\s)", RegexOptions.IgnoreCase).Match(temp).Value);
            }
            if (new Regex(@"(?<=тел:\s*?)\+[\d\-]+(?=\s+\w+)", RegexOptions.IgnoreCase).IsMatch(temp))
            {
                string[] manager = new string[]
                {
                    new Regex(@"(?<=тел:\s*?\+[\d\-]+\s+)\w+", RegexOptions.IgnoreCase).Match(temp).Value, //имя
