...
{
    class Class_cxml_UTK_Stal_EKB_Excel
    {
                        ...
                        for (int j = tab.StartRow; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (new Regex(@"^\d+?\s(?=\w)", RegexOptions.IgnoreCase).IsMatch(temp))
                                        temp = temp.Replace(new Regex(@"^\d+?\s(?=\w)", RegexOptions.IgnoreCase).Match(temp).Value, "");

                                    if (regexParam.RegName.IsMatch(temp))
                                    {
                                        tab.Name = regexParam.RegName.Match(temp).Value;
                                        temp = temp.Replace(tab.Name, "");
                                    }
                                    if (new Regex(@"\bшест(?:.+?р.+?ник)?\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                    {
                                        tab.Name = "Шестигранник";
                                        temp = temp.Replace(new Regex(@"\bшест(?:.+?р.+?ник)?\b", RegexOptions.IgnoreCase).Match(temp).Value, "");
                                    }
                                    if (string.IsNullOrEmpty(tab.Name)) tab.Name = "Труба";

                                    if (regexParam.RegType.IsMatch(temp) && !string.IsNullOrEmpty(tab.Name))
                                    {
                                        tab.Type = "";
                                        ...
                                        if (!string.IsNullOrEmpty(tab.Type)) tab.Type = regexParam.SetRightEnding(tab.Type, tab.Name);
                                        else tab.Type = regexParam.RegTypeShort(temp, tab.Name);
                                        if (!string.IsNullOrEmpty(tab.Type))
                                            temp = temp.Replace(tab.Type, "");
                                    }
                                    ...
                                    if (!string.IsNullOrEmpty(tab.Name))
                                    {
                                        ...
                                        if (tab.Name.ToLower().Contains("лист") || tab.Name.ToLower().Contains("круг"))
                                        {
                                            diam = new Regex(@"\d+(?:[,\.]\d+)?(?=\s*?мм)", RegexOptions.IgnoreCase).Match(temp).Value;
                                            if (!string.IsNullOrEmpty(diam)) temp = temp.Replace(diam, "");
