                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                ...
                                if (new Regex(@"норм.*документ", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColGost = i;
                                    continue;
                                }
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            {                                
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (string.IsNullOrEmpty(diam) && string.IsNullOrEmpty(standart))
                                    {
                                    ...
                                    else if (!string.IsNullOrEmpty(standart))
                                    {
                                        temp = standart;
                                        //diamDxDxD
                                        if (new Regex(@"\d+(?:[,.]\d+)?\s*[xх\*×]\s*\d+(?:[,.]\d+)?\s*[xх\*×]\s*\d+(?:[,.]\d+)?", RegexOptions.IgnoreCase).IsMatch(temp))
                                        ...
                                        if (new Regex(@"(?<=ду\s*)\d+(?=\w|\s|$)", RegexOptions.IgnoreCase).IsMatch(temp))
                                            diam = new Regex(@"(?<=ду\s*)\d+(?=\w|\s|$)", RegexOptions.IgnoreCase).Match(temp).Value;
                                        prim = diam;
                                        if (!string.IsNullOrEmpty(tolsh)) prim += "x" + tolsh;
                                        if (!string.IsNullOrEmpty(metraj)) metraj += "x" + metraj;
                                    }
                                    else
                                    {
                                        prim = diam;
                            ...
                            if (!String.IsNullOrEmpty(diam) && (!string.IsNullOrEmpty(standart) || !string.IsNullOrEmpty(mera) || !string.IsNullOrEmpty(mark)))
                            {
