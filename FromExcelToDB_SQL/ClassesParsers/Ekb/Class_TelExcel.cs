...
namespace MetalBase
{
    class Class_TelExcel
    {
        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    int ColRaz = 0, ColMera = 0, ColPrice = 0, ColGost = 0, ColSoder = 0, ColDiam = 0, ColTolsh = 0, ColMark = 0;
                    ...
                    {
                        ColPrice = 0; ColRaz = 0; ColMera = 0; ColSoder = 0; ColGost = 0;
                        name = ""; type = "";
                        Excel.Range cellRange;
                        tab = tabs[k];
                        int endRow = cCelRow;
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"\bразмер\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    if (ColRaz == 0)
                                    {
                                        ...
                                        {
                                            cellRange = (Excel.Range)excelworksheet.Cells[jjj - 1, 1];
                                            if (cellRange.Value != null)
                                                temp = cellRange.Value.ToString().Trim();
                                            else temp = "";
                                        }
                                ...
                                if (new Regex(@"\bсвободно\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    if (ColMera == 0)
                                    {
                                        ColMera = i;
                                ...
                                if (new Regex(@"\bцена\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ...
                                    {
                                        ColPrice = i;
                                        cellRange = (Excel.Range)excelworksheet.Cells[jjj + 1, ColRaz];
                                        if (cellRange.MergeArea.Columns.Count > 1)
                                        {
                                            for (int ii = cellRange.MergeArea.Column; i <= cellRange.MergeArea.Columns.Count - 1; ii++)
                                            {
                                                ...
                                                if (temp != "")
                                                {
                                                    if (new Regex(@"руб/тн", RegexOptions.IgnoreCase).IsMatch(temp)) ColPrice = ii;
                                                }
                                            }
                                            continue;
                                        }
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            if (ColRaz > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (!String.IsNullOrEmpty(diam) && !string.IsNullOrEmpty(price))
                                    {
        ...
        private static void FillInfoOrg(InfoOrganization infoOrg, string temp, C_RegexParamProduct regexParam, Excel.Worksheet worksheet, int j, int i)
        {
            ...
            {
                infoOrg.OrgAdress = new Regex(@"(?:\d{6}\s*,?\s*)?(г\.\s*\w+,?)?(?:(?:\s*(?:ул|пр(?:-к?т)?|д|оф)\.\s*\w+(?:\s*\w+)?,?)+|\s\d+/\d+,?)+", RegexOptions.IgnoreCase).Match(temp).Value;
            }
