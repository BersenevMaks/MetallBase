...
{
    class Class_RosTehKom_160819_Excel
    {
        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    for (int j = 1; j <= 15; j++)//cCelRow; j++) //строки
                    {
                        int jj = j;
                    ...
                    for (int k = 0; k < tabs.Count; k++)
                    {
                        ...
                        for (int i = tab.StartCol; i < endCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"склад|номенклатура", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColDiam = i;
                                    continue;
                                }
                                ...
                                //    continue;
                                //}
                                if (new Regex(@"\bсвоб.?остаток\b", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    ColMera = i;
                                    continue;
                                }
                        ...
                        for (int j = tab.StartRow; j <= endRow; j++)
                        {
                            ...
                            if (ColDiam > 0)
                            {
                                ...
                                if (temp != "")
                                {
                                    ...
                                    if (regexParam.RegType.IsMatch(temp) && !string.IsNullOrEmpty(name))
                                    {
                                        type = "";
                                        ...
                                        if (!string.IsNullOrEmpty(type)) type = regexParam.SetRightEnding(type, tab.Name);
                                        else
                                        {
                                    ...
                                    if (new Regex(@"г[\/\\]к", RegexOptions.IgnoreCase).IsMatch(temp))
                                        type = "горячекатаный";
                                    if (new Regex(@"х[\/\\]к", RegexOptions.IgnoreCase).IsMatch(temp))
                                        type = "холоднокатаный";
