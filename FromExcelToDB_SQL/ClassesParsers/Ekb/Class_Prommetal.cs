        ...
        private void ReadExcel()
        {
            ...
            Type.Missing, Type.Missing);
                ...
                foreach (Excel.Worksheet excelworksheet in excelsheets)
                {
                    ...
                    int ColRazDl = 0, ColDlina = 0, ColMera = 0, ColPrice = 0, ColMark = 0, ColTolsh = 0, ColProfil = 0, ColSechenie = 0, ColName = 0;
                    ...
                    for (int j = 4; j <= cCelRow; j++) //строки
                    {
                        ...
                        for (int i = 1; i <= cCelCol; i++) //столбцы
                        {
                            ...
                            if (temp != "")
                            {
                                if (new Regex(@"^категория", RegexOptions.IgnoreCase).IsMatch(temp))
                                {
                                    tab = new C_InfoTable
                                    {
                                        StartCol = i,
                    ...
                    {
                        ColMark = 0; ColPrice = 0; ColDlina = 0; ColMera = 0; ColRazDl = 0; ColProfil = 0; ColSechenie = 0; ColName = 0;
                        name = ""; type = "";
                        Excel.Range cellRange;
                        tab = tabs[k];
                        int endRow = cCelRow;
                        ...
                        for (int j = tab.StartRow + 1; j <= endRow; j++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(name))
                            {
                                if (new Regex(@"швелл?ер", RegexOptions.IgnoreCase).IsMatch(temp)) type = new Regex(@"(?<=\d\s*)(?:б|м|ш|kк|[уy]|п)", RegexOptions.IgnoreCase).Match(temp).Value;
                                if (ColMark > 0)
                                {
